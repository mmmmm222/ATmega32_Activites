...
//----------------------
		...
		SET_BIT(TCCR0,TCCR0_WGM01);
	#if FAST_PWM_MODE == CLR_ON_COMPARE
		CLR_BIT(TCCR0,TCCR0_COM00);
		SET_BIT(TCCR0,TCCR0_COM01);
	#elif FAST_PWM_MODE == SET_ON_COMPARE
		SET_BIT(TCCR0,TCCR0_COM00);
		SET_BIT(TCCR0,TCCR0_COM01);

	#elif FAST_PWM_MODE == DISCONNECTED
		CLR_BIT(TCCR0,TCCR0_COM00);
		CLR_BIT(TCCR0,TCCR0_COM01);

	#else
	#error "Wrong FAST PWM MODE choice"
...
void SetCompareMatchValue(u8 Copy_Value){
		OCR0 = Copy_Value;
	}
